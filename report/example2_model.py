...
class RelationAttn(nn.Module):
    ...
    def __init__(self, v: int, init_with_zeros=False):
        super().__init__()
        self.v = v
...
class Example1(nn.Module):
    ...
    def __init__(self, v, inverse_class_probs=None, init_with_zeros=False):
        ...
        super().__init__()
        self.v = v
        self.reln_attn = RelationAttn(v, init_with_zeros=init_with_zeros)
...
class RelationAttnLayers(nn.Module):
    ...
    def __init__(self, v: int, num_layers: int,
                 init_with_zeros):
        super().__init__()
        self.v = v
        ...
        for _ in range(num_layers):
            attention_layer = RelationAttn(v, init_with_zeros=init_with_zeros)
...
class Example2(nn.Module):
    def __init__(self, v, num_layers: int, inverse_class_probs=None, init_with_zeros=False):
        ...
        super().__init__()
        self.v = v
        self.attn_layers = RelationAttnLayers(v, num_layers, init_with_zeros)
...
def example1a():
    ...
    dataset = Data(seqs, labels)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=10)
    model = Example2(v, num_layers, init_with_zeros=False)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-2)
    # max_steps = 30
    num_epochs = 10000
    print_freq = 2000
